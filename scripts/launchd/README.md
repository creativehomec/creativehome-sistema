...
O instalador copia o script pra `~/Library/Application Support/creativehome/`,
**move** a credencial pra `~/.config/creativehome/env` com permissão 600, e
carrega o agente.
...
```bash
launchctl kickstart -p gui/$(id -u)/com.creativehome.backup
```

```bash
launchctl list | grep creativehome
```
...
```bash
launchctl start com.creativehome.backup
```

## Desativar

```bash
launchctl unload ~/Library/LaunchAgents/com.creativehome.backup.plist
```
...
```bash
gunzip -c ~/Backups/creativehome/creativehome-AAAA-MM-DD.sql.gz | /opt/homebrew/opt/libpq/bin/psql "$SUPABASE_DB_URL"
```

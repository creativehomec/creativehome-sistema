...
DESTINO="$HOME/Backups/creativehome"
MANTER_DIAS=7
...
if [ -z "${CONFIG:-}" ]; then
  ...
  echo "  \$BACKUP_CONFIG, ~/.config/creativehome/env, $PROJETO/.env.backup" >&2
  exit 1
fi
...
DATA="$(date +%Y-%m-%d)"
ARQUIVO="$DESTINO/creativehome-$DATA.sql.gz"
...
find "$DESTINO" -name 'creativehome-*.sql.gz' -type f -mtime "+$MANTER_DIAS" -delete

RESTANTES=$(find "$DESTINO" -name 'creativehome-*.sql.gz' -type f | wc -l | tr -d ' ')

...
| Estrutura do banco | `supabase/setup.sql` e `supabase/migrations/` | Completa. As 29 tabelas se recriam do zero. |
| Dados | Só no Supabase | **Nenhuma.** Ver "O que não está protegido". |

Se o projeto do Supabase sumisse, dava pra criar um novo, rodar o
`setup.sql` e ter o sistema de pé — vazio, mas inteiro.
...
2. Rodar `supabase/setup.sql` inteiro no SQL Editor. Isso monta as 29 tabelas
...
`~/Backups/creativehome/`, um arquivo por dia, mantendo sete.
